...
router.get('/listado-citas', (req, res) => {
  conexion.query(`SELECT fecha_cita, pacientes.nombres, pacientes.apellidos, pacientes.telefono, medicos.especialidad, medicos.consultorio, medicos.nombres nombreMedico, medicos.apellidos apellidoMedico FROM cita_medica, pacientes, medicos WHERE cedula_medico=medicos.cedula AND  cedula_paciente=pacientes.cedula;`, (error, resultado) => {
    if (error){
      console.log('Ocurrio un error en la ejecucion', error)
      res.status(500).send('Error en la ejecucion')
    }else{
...
router.post('/agregar-medico', (req, res) => {
  ...
  //res.status(200).send('Agregando médico')
})
...
//Enrutamiento para consultar los medicos de una especialidad y poder agendar una cita medica
